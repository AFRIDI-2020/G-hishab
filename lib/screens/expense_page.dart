...
class _ExpensePageState extends State<ExpensePage> {
  ...
  SizedBox _bodyUI(Size size, DatabaseHelper databaseHelper) => SizedBox(
    width: size.width,
    height: size.height,
    child: Stack(
      children: [
                    ...
                    Expanded(
                      child: RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemCount: databaseHelper.expenseList.length,
                          itemBuilder: (context, index) {
                            DateTime miliDate =
                            DateTime.fromMillisecondsSinceEpoch(int.parse(databaseHelper.expenseList[index].date));
                            var format = DateFormat("yMMMd").add_jm();
                            String finalDate = format.format(miliDate);
                            String amountInBn = enToBnNumberConvert(databaseHelper.expenseList[index].amount);
                            return GestureDetector(
                              onLongPress: (){
                                showUpdateDeleteDialog(size, databaseHelper,
                                    databaseHelper.expenseList[index], index);
                              },
                              child: IncomeExpenseDetailPreview(
                                date: finalDate,
                                description: databaseHelper.expenseList[index].description,
                                amount: amountInBn,
                              ),
                            );
                          },
                        ...
                        Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Text(
                                '${databaseHelper.totalExpenseString.value} টাকা',
                                style: TextStyle(
                                    color: Colors.black,
                                    fontSize: size.width * .04),
                              ),
  ...
  void addExpense() async {
    if (_descriptionController.text.isEmpty) {
      showToast('ব্যয়ের খাত লিখুন');
      return;
    }
    if (_amountController.text.isEmpty) {
      showToast('ব্যয়ের পরিমাণ লিখুন');
      return;
    }
  ...
  void showUpdateDeleteDialog(Size size, DatabaseHelper databaseHelper,
    ...
    _updateDescriptionController.text =
        databaseHelper.expenseList[index].description;
    String amount = enToBnNumberConvert(databaseHelper.expenseList[index].amount);
    _updateAmountController.text = amount;
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    GestureDetector(
                      onTap: () async {
                        String amount = bnToEnNumberConvert(_updateAmountController.text);
                        await databaseHelper.updateEntry(IncomeExpenseModel(
                            databaseHelper.expenseList[index].entryId,
                            databaseHelper.expenseList[index].date,
                            amount,
                            _updateDescriptionController.text,
                            databaseHelper.expenseList[index].status));
                        Get.back();
                      },
                      child: Container(
                        padding: EdgeInsets.all(size.width * .03),
                ...
                GestureDetector(
                  onTap: () async {
                    await databaseHelper.deleteEntry(
                        databaseHelper.expenseList[index].entryId,
                        databaseHelper.expenseList[index].status,
                        index).then((value){
                      Get.back();
